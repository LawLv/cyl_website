          ...
          {experiences.map((exp, index) => (
            <div key={index} className={`p-6 bg-white border-l-4 ${getTypeColor(exp.type)} hover:shadow-lg transition-all duration-300`}>
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="text-xs text-gray-500 font-medium bg-gray-100 px-2 py-1 rounded">{exp.period}</span>
              </div>
              
              <p className="text-gray-600 text-sm mb-4 leading-relaxed line-clamp-3">{exp.description}</p>
              ...
              <div className="flex flex-wrap gap-1 mb-4">
                {exp.skills.slice(0, 4).map((skill, skillIndex) => (
                  <span
                    key={skillIndex}
                    className="px-2 py-1 bg-cyan-100 text-cyan-800 text-xs rounded font-medium"
                  >
                    {skill}
                  </span>
                ))}
              ...
              {exp.link && (
                <a
                  ...
                  className="inline-flex items-center px-3 py-1 bg-gray-800 text-white text-xs font-medium rounded hover:bg-gray-700 transition-colors duration-200"
                >
